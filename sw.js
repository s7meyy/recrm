...
// ٢) **الشبكة أولًا لكل شيء، والمخزن احتياطٌ للعمل دون اتصال.** كان الاحتياط أولًا لملفات
//    التطبيق (JS/CSS) طلبًا للسرعة، وكانت تلك علّة صامتة: المخزن لا يُبطَل أبدًا (اسمه ثابت)،
//    فأي تحديث في CSS أو JS لا يصل إلى المتصفح إطلاقًا ما دامت فيه نسخة قديمة — تُنشر
//    المرحلة ولا يرى صاحبها شيئًا. والسرعة لم تضع: الملفات تُعاد بـ304 من ذاكرة المتصفح.
//
...
self.addEventListener('fetch', (event) => {
  ...
  event.respondWith((async () => {
    ...
    // ملفات التطبيق: الشبكة أولًا كي يصل كل تحديث فور نشره، والمخزن إن انقطعت الشبكة.
    try {
      const fresh = await fetch(request);
