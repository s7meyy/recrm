...
## ٦٤. المرحلة ٥٧ — ما كشفته المراجعةُ بعين العميل: اثنا عشر بندًا

بعد المرحلة ٥٦ تُصفِّح الموقعُ كلُّه — اثنتان وثلاثون صفحةً على مقاسَي الحاسوب والجوّال،
ثم الصفحاتُ العامة كما يفتحها العميل — وخرجت خمسةٌ وثلاثون ملاحظة. اعتمد صاحبُ المكتب
ترتيبَها، وهذه المرحلةُ تأخذ أعطابَ الشكل الثمانية (أ) وأربعةَ الجوّال الأثقلَ أثرًا (ج).

### أ. الأعطاب الثمانية

1. **الفرص العقاريّة — التنبيه فقرةً.** `.notice` صفٌّ مرنٌ بـ`space-between`، فكان
   يرمي «بابٌ لم يُفتح» إلى أقصى اليمين و«فأنت مسؤول» إلى أقصى اليسار من السطر نفسه
   والباقي تحتهما، فتُقرأ الجملة بغير ترتيبها. `notice-prose` تجعله كتلةً تُقرأ من أوّلها.
2. **الصفحة العامة — العدّاد يُجمع.** «4 عرض» و«4 listing» في أوّل سطرٍ يراه العميل.
   الآن `listings(n, shown)` في ملفّ اللغة على `countAr` نفسها: عرضٌ واحد · عرضان ·
   ٤ عروض · ١١ عرضًا، و`1 listing / 4 listings`.
3. **الفواتير — الفلاتر في صفّ.** كانت القائمتان كلٌّ بعرض الصفحة فوق بعضهما لأن
   `.input` عرضُها ١٠٠٪. `filter-bar`: بحثٌ يتمدّد وقوائمُ بعرضها الطبيعي.
4. **الاستمارة العامة — القوائم منسّقة.** `select` كانت خارج قاعدة الحقول فتظهر خامًا
   ملتصقةً بعنوانها بين حقولٍ منسّقة — وهي أوّل ما يملؤه عميلٌ من جواله. صارت كالحقول
   بارتفاع ٤٤ وسهمٍ مرسوم.
5. **الاستمارة العامة — بطاقةُ نجاح.** بعد الإرسال كانت جملةٌ تحت الزرّ والحقولُ باقية
   فارغة، فلا يُدرى أأُرسل شيء. الآن تختفي الاستمارة وتظهر بطاقة «وصلنا طلبك يا فلان»
   وفيها واتساب المكتب (إن عُرف جوالُه من النشرة) و«تصفّح العروض» و«طلب آخر».
6. **«الإعدادات» الثابت يغطّي ما تحته.** كان رابطًا مدوَّرَ الزوايا، فمن الزاويتين يظهر
   رأسُ المجموعة المارُّ تحته مقطوعًا. زواياه مستقيمة وخلفيته صلبة تمتدّ هامشًا فوقه.
7. **صفّ العميل** — رُوجع فتبيّن أن النقر على الصفّ **يفتح الملفّ فعلًا** (نافذة لا
   مسار)؛ ما رأته المراجعة كان نقرةً على رابط الجوال الذي يوقف الانتشار بقصد. لم يُمسّ،
   ويُثبته الاختبار.
8. **حقل التاريخ الفارغ.** `mm/dd/yyyy` يبقى عند من لغةُ جهازه إنجليزية، والشرحُ تحته لا
   يمنع رؤيته. `coverEmptyDate` في `echoDates`: غطاءٌ يقول «اختر التاريخ» ما دام الحقل
   فارغًا وغيرَ مركَّز، ويزول بالتركيز أو القيمة — فلا تُخسر لوحةُ التاريخ في الجوّال.

### ب. الجوّال الأربعة

17. **شريط النسخة سطرٌ واحد.** كان ثلاثةَ أسطرٍ وزرّين بارتفاع ١٣٠ بكسلًا فوق عنوان كلّ
    صفحة. `banner-slim`: سطرٌ لا يلتفّ، والشرحُ في `title`، والارتفاع دون ٦٠.
18. **بطاقةُ العميل والطلب المضغوطة.** الجدولُ على الجوّال بطاقةٌ من ثمانية أسطر
    «عنوان: قيمة»، وعشرةُ عملاء ٣٤٠٠ بكسلًا. الآن: **الاسمُ العمودُ الأوّل** (فهو عنوان
    البطاقة، وكانت «الأولوية» فتتصدّرها شرطة)، والثانويُّ `m-hide` يُطوى (الأولوية،
    الأدوار، التصنيفات، المسند إليه؛ وفي الطلبات المدينة والمساحة)، وصفُّ `m-actions`
    يظهر بزرّي اتصالٍ وواتساب بحجم ٤٤ لا تُرى على الشاشة الواسعة، ورقمُ الجوال نفسه هدفٌ
    ٤٤. وواتساب من `openWhatsApp` فيُسجَّل تواصلًا.
19. **العقارات عمودًا واحدًا.** عمودان في ٣٩٠ بكسلًا كسرا «بلا موقع» و«٧٬١١١ ريال/م²»
    على أربعة أسطر.
23. **الميكروفون في الزاوية.** كان ٥٢ بكسلًا في منتصف الجانب يغطّي أزرار البطاقات؛ صار
    ٤٤ في الزاوية السفلى. (وقاعدته بعد `.voice-btn` كي تغلبها — كانت قبلها فلا تعمل.)

### ج. الملفات المعدَّلة وسببها

| الملف | لماذا |
| --- | --- |
| `js/pages/prospects.js` · `css/components.css` | `notice-prose` |
| `offers/i18n.js` · `offers/app.js` | `listings(n)` يُجمع |
| `js/pages/invoices.js` · `css/components.css` | `filter-bar` |
| `offers/style.css` · `offers/intake.js` | القوائم منسّقة · بطاقة النجاح |
| `css/base.css` | «الإعدادات» الثابت بلا زوايا · `banner-slim` |
| `js/util/dom.js` · `css/components.css` | `coverEmptyDate` |
| `js/app.js` | الشريط سطرًا واحدًا |
| `js/pages/clients.js` · `js/pages/requests.js` · `css/components.css` | `m-hide` · `m-actions` · الاسم أوّلًا |
| `css/components.css` | `.grid` عمودٌ واحد · الميكروفون |
| `tests/review-fixes.mjs` (جديد) · `tests/review-unit.mjs` (جديد) · `tests/run.mjs` | كلُّ بندٍ يُقاس بالبكسل |

### د. لم يُمسّ

- **البنود ٩–١٦ و٢٠–٣٥** من المراجعة — للمرحلة التالية بترتيبها المعتمَد.
- **الجداول على الشاشة الواسعة** — `m-hide` و`m-actions` لا أثر لهما فوق ٧٢٠ بكسلًا إلا
  أنّ الاسم صار قبل الأولوية في جدول العملاء.
- **منتقي التاريخ الأصليّ** — الغطاءُ فوقه لا بدلَه.

## ٦٥. المرحلة ٥٨ — الصفحة العامة بعين العميل: البنود ٩–١٦

الدفعةُ الثانية من مراجعة المرحلة ٥٧: ما يراه **العميلُ** لا صاحبُ المكتب — قائمةُ العروض،
وصفحةُ العرض، والحجزُ، والاستمارة. والصفحاتُ العامة مستقلّةٌ عن التطبيق (لا تصل IndexedDB)،
فكلُّ ما هنا في `offers/` و`netlify/functions/offer.js` وتنبيهٍ واحدٍ في صفحة النشر.

### أ. ما تغيّر

9. **العرض بلا صور.** كان مربّعًا بارتفاع الصورة فيه اسمُ النوع وحده، وفي صفحة العرض
   مربّع «لا صور» بارتفاع الشاشة. صار **شريطًا قصيرًا** (`card-media-none` /
   `noimg-slim`) يقول «بلا صور بعد» ويقترح المعاينة. **وقبل النشر يُقال بالعدد**
   (`passQualityGate`): «٣ من ٤ عروض بلا صور، وواحد بلا سعر — تنشر على كل حال؟». القرارُ
   قرارُه، لكن لا يكون بلا علم. و`qualityGaps` دالّةٌ خالصة.
10. **لا تكرارَ للمكان.** «فلة — الملقا، الرياض» ثم «الملقا، الرياض» تحته في كل بطاقة
    وفي صفحة العرض. سطرُ المكان أُزيل حيث يحمله العنوان (البطاقة، صفحة العرض، القائمة
    الخاصّة).
11. **المشاركة من صفحة العرض.** كان «شارك» في القائمة وحدها، وصفحةُ العرض هي ما يُرسل.
    زرٌّ يستعمل مشاركةَ الجهاز، وإلا ينسخ الرابط ويقول «نُسخ الرابط»، وإلا واتساب بالنصّ.
12. **«اطلب معاينة» حين يكون الحجز مغلقًا** — في البطاقة وفي صفحة العرض — يفتح الاستمارة
    بـ`?ref=` فتقول «بخصوص العرض رقم ٣» وتُرسل الرقم مع الطلب، فيصل صاحبَ المكتب موصولًا
    بعرضه. (كان الزرُّ لا يظهر إلا والحجزُ مفتوح، فيبقى للعميل واتساب أو اتصال.)
13. **الحجز مغلق.** كانت جملةً تحيل إلى «المكتب» بلا زرّ. الآن ثلاثةُ أزرار: واتساب
    (بنصٍّ فيه رقم العرض إن جاء منه)، اتصال، واستمارة برقم العرض.
14. **هويّة واحدة.** صفحةُ العرض الواحد لم تكن تحمل اسمَ المكتب ولا شعارَه في ترويستها.
    صار فيها `office-line`: الشعار والاسم والجوال، كالقائمة والحجز والاستمارة.
15. **الإنجليزية.** الأسماءُ العربيّة (الحي والمدينة) داخل الجملة الإنجليزية تُعزل بـ`<bdi>`
    فلا تقلب ترتيبَها؛ والعدّاد «4 listings»؛ وخياراتُ السعر «Up to 1M». **وحدُّه**: الأحياءُ
    تبقى بحروفها العربية — لا قاموسَ نقحرةٍ عندنا، واختراعُه خطأٌ أسوأ من العربية.
16. **فلاتر السعر والمساحة والمفضّلة.** «السعر: حتى X» و«المساحة: X فأكثر» — درجاتُ سلّمٍ
    ثابت لا يُعرض منها إلا ما **يفصل بين العروض فعلًا**، فلا قائمةَ من عشر درجاتٍ تُعطي
    النتيجةَ نفسها. **وبلا سعرٍ لا يُقصى بفلتر السعر**: عرضٌ سعرُه عند الطلب قد يكون في
    الميزانية. والمفضّلةُ قلبٌ ٤٤ بكسلًا على كل بطاقة، تُحفظ في متصفّح العميل وحده
    (`localStorage`) ولا تصل الخادم ولا تُطلب بها هويّة، وزرٌّ «المفضّلة (n)» يُريه ما حفظ.
    **والمقارنة لم تُبنَ** — جدولُ مقارنةٍ على جوّالٍ بعرض ٣٩٠ سؤالُ تصميمٍ يُطرح على
    صاحب المكتب لا يُفترض.

### ب. الملفات المعدَّلة وسببها

| الملف | لماذا |
| --- | --- |
| `offers/app.js` | بلا صور شريطًا · لا تكرار · `<bdi>` · «اطلب معاينة» · فلاتر السعر والمساحة · المفضّلة |
| `offers/i18n.js` | سلاسل المرحلة بالعربية والإنجليزية |
| `offers/style.css` | الشريط القصير · القلب · هويّة المكتب · أزرار الحجز المغلق · شارة رقم العرض |
| `netlify/functions/offer.js` | هويّة المكتب · `<bdi>` · بلا صور قصيرًا · «شارك» · «اطلب معاينة» |
| `offers/book.js` | الحجز المغلق بأزراره |
| `offers/intake.js` | `?ref=` يُقال ويُرسل |
| `offers/list.js` | لا تكرار للمكان |
| `js/pages/publish.js` | `qualityGaps` · `passQualityGate` قبل النشر |
| `tests/public-experience.mjs` (جديد) · `tests/run.mjs` | كلُّ بندٍ كما يراه العميل على ٣٩٠ بكسلًا |
| `tests/gate-and-publish.mjs` · `tests/client-links.mjs` · `tests/offer-pwa-push.mjs` | بابُ الجودة يُجاب في التجربة |

### ج. لم يُمسّ

- **البيانات المنشورة** — اللقطةُ كما هي؛ لا حقلَ جديدًا فيها، والمفضّلةُ عند العميل لا عندنا.
- **الحجز المفتوح** ومساره — تغيّر المغلقُ وحده.
- **البنود ٢٠–٣٥** من المراجعة — للمرحلة التالية.

## ٦٦. المرحلة ٥٩ — الدفعة الثالثة من المراجعة: الجوّال والحاسوب (ج/د)

البنود ٢٠–٢٢ و٢٤–٣٠ من مراجعة المرحلة ٥٧. جامعُها: **ما يُقرأ أوّلًا يُرى أوّلًا، وما
عداه يُطوى خلف زرٍّ يسمّي عددَه** — لا يُحذف ولا يُخفى صامتًا.

### أ. ما تغيّر

20. **الداشبورد على الجوّال** (`foldRestOnNarrow`): قِيس فبلغ ٨٠٠٠ بكسل. تُعرض أربعُ
    لوحاتٍ أُولى ويُطوى الباقي خلف «المزيد — ١٤ لوحة أخرى». والفارغةُ تُطوى قبله في سطرها
    كما كانت (المرحلة ٥٢). وعلى الشاشة الواسعة لا شيء.
21. **الإعدادات على الحاسوب أيضًا**: كانت ١٦٬٠٠٠ بكسل بكل اللوحات مفتوحة. `foldOnMobile`
    يطوي على كل الشاشات، **ويتذكّر ما فتحتَه** في هذا الجهاز (`kassab:settings-open`)؛
    و«افتح الكل / اطوِ الكل» في الفهرس لمن يفضّل الصفحة الطويلة (تُحفظ `'*'`). والبحثُ يفتح
    ما يوافقه كما كان. وقواعدُ `.set-fold` خرجت من استعلام الجوّال إلى كل الشاشات.
22. **التقويم على الجوّال** (`agendaList`): شبكةُ الشهر ثلاثون صفًّا أكثرُها فارغ. تُعرض
    الأيامُ التي فيها شيءٌ وحدها، يومًا فيومًا، بمواعيده كاملةً وكلٌّ هدفُ لمسٍ ٤٤.
24. **أهداف اللمس التي بقيت**: أسهمُ المهامّ وزرُّ «+» (٢٩ ← ٤٤ عرضًا)، روابطُ الهيئة (٢٢
    ← ٤٤ ارتفاعًا)، مربّعُ الاختيار بلا نصّ (`check-bare`: الغلافُ ٤٤ والمربّعُ ٢٢)،
    وأزرارُ تكبير الخريطة (٣٠ ← ٤٤).
25. **تنبيه البيانات التجريبية**: فُحص فتبيّن أنه يزول بعد ستّ ثوانٍ كما صُمّم؛ ما رأته
    المراجعةُ كان لقطاتٍ في أوّل ثوانيه. لم يُمسّ، والاختبار يُثبت زوالَه.
26. **المطابقات**: أعلى ثلاثٍ لكل طلبٍ ظاهرة، والباقي خلف «أظهر ١٦ مطابقة أخرى». الصفوفُ
    مرتّبةٌ بالدرجة أصلًا فالثلاثُ الأولى هي ما يُقرَّر به.
27. **الوارد والتكاملات**: التنبيهُ سطرٌ واحدٌ يقول الحكمَ («لا يدخل شيءٌ قاعدتَك حتى تضغط
    «اعتمده»»)، والشرحُ في `disclosure` مطويّ. (واتساب كان مطويًّا منذ المرحلة ٤٢.)
28. **العروض الخارجية**: الصفُّ الناقص فيه زرُّ «أكمل الآن» يفتح استمارته.
29. **الأفكار**: سطرٌ يقول إن ما يُوسم في الوارد «فكرة/مقترح» يُقيَّد هنا، برابطٍ إليه.
30. **المالية**: الأشهرُ بلا حركة تُطوى (`month-quiet`) ويُقال عددُها في زرّ؛ الشهرُ
    الحاليُّ يبقى دائمًا.

### ب. الملفات المعدَّلة وسببها

| الملف | لماذا |
| --- | --- |
| `js/pages/dashboard.js` | `foldRestOnNarrow` |
| `js/pages/settings.js` · `css/components.css` | الطيُّ على كل الشاشات · التذكّر · افتح/اطوِ الكل |
| `js/pages/calendar.js` · `css/components.css` | `agendaList` وأنماطها |
| `js/util/dom.js` · `css/components.css` | `check-bare` · أسهم المهامّ · روابط الهيئة · تكبير الخريطة |
| `js/pages/matches.js` | أعلى ثلاثٍ ثم `match-more` |
| `js/pages/inbox.js` · `js/pages/integrations.js` | سطرٌ + `disclosure` |
| `js/pages/external.js` | «أكمل الآن» |
| `js/pages/notes.js` | سطرُ الوارد |
| `js/pages/expenses.js` | `month-quiet` |
| `tests/review-round3.mjs` (جديد) · `tests/run.mjs` | كل بندٍ بالبكسل والعدّ |
| ثلاثة عشر اختبارًا قديمًا | تفتح كل لوحات الإعدادات (`'*'`) قبل أن تقرأها |

### ج. لم يُمسّ

- **البنود ٣١–٣٥** (إضافات): بطاقة المشاركة، زرّ التواصل الموحّد، مؤشّر المزامنة — تُطرح
  على صاحب المكتب مرحلةً مستقلّة.
- **الطيُّ على الجوّال في الإعدادات** — كما كان منذ المرحلة ٥٢، وزاد عليه التذكّر.

## ٦٧. المرحلة ٦٠ — الإضافات (هـ) من مراجعة المرحلة ٥٧

البنود ٣١ و٣٣ و٣٤ و٣٥. (والبند ٣٢ — تنبيهُ النشر بلا صور وبلا سعر — أُنجز في المرحلة ٥٨.)

### أ. ما أُضيف

31. **بطاقةُ المشاركة** (`js/util/share-card.js`): من ملفّ العقار زرُّ «🖼️ بطاقة مشاركة»
    يرسم على `<canvas>` صورةً ١٠٨٠×١٠٨٠ — صورةُ العقار (أو النوعُ إن لم توجد)، والسعرُ
    عليها، والعنوانُ والحقائقُ رقائقَ، واسمُ المكتب وجوالُه وشعارُه — ونصًّا مرافقًا فيه
    رابطُ العرض إن كان منشورًا. **لا تصل الشبكة**: الصورةُ والشعارُ من مخزن الجهاز. والأزرار:
    «شارك» يمرّ بمشاركة الجهاز حاملًا الملفَّ والنصّ حيث تُدعم (الجوّال)، وإلا يحمّل الصورةَ
    وينسخ النصّ؛ و«حمّل الصورة»؛ و«انسخ النصّ». `drawShareCard` و`cardText` و`cardFacts`
    دوالٌّ تُختبر وحدها.
33. **زرُّ «تواصل» الموحّد** (`contactMenu` في `outreach.js`): في رأس ملفّ العميل وفي
    نافذة تفاصيله زرٌّ واحد يفتح قائمة: اتصال · واتساب (يسجّل تواصلًا مستنتَجًا كما كان) ·
    سجّل تواصلًا (يقفز إلى الاستمارة). كلُّ خيارٍ ٤٤، والنقرُ خارجها يغلقها. وبطاقاتُ
    الجوّال في جدول العملاء تبقى بزرّيها الكبيرين — هناك الإبهامُ يريد زرًّا لا قائمة.
34. **مؤشّرُ النسخة في الرأس** (`#backup-dot`): أيقونةٌ صغيرة بجانب نقطة المزامنة، تحمل
    نقطةً خضراء دون يوم، صفراء دون ثلاثة، حمراء بعدها أو إن لم تُحفظ نسخةٌ قطّ؛ والنصُّ
    في عنوانها؛ والنقرُ يفتح الإعدادات. **والشريطُ الأصفر صار بعد ثلاثة أيام لا يوم**
    (`BANNER_HOURS = 72`، و`bannerDue`)، ولمن لم يحفظ قطّ — فالمؤشّرُ يقول الحالَ قبله بلا
    أن يحجب شيئًا. و`REMINDER_HOURS` (يوم) باقيةٌ لِما سواه.
35. **يومي يطوي الفارغ**: `foldEmptyPanels` نفسُها (المرحلة ٥٢) تُستدعى في «يومي» بعنوان
    «٦ لوحات لا شيء فيها اليوم» وزرّ «أظهرها». والداشبوردُ والإعداداتُ كانا يطويان قبلها.

### ب. الملفات المعدَّلة وسببها

| الملف | لماذا |
| --- | --- |
| `js/util/share-card.js` (جديد) · `js/pages/property.js` · `css/components.css` | البطاقة ونافذتها وزرّها |
| `js/util/outreach.js` · `js/pages/client.js` · `js/pages/clients.js` · `css/components.css` | `contactMenu` |
| `js/data/backup.js` · `js/app.js` · `index.html` · `css/components.css` | `bannerDue` · `dot` · `paintBackupDot` |
| `js/pages/dashboard.js` · `js/pages/today.js` | `foldEmptyPanels` بعنوانٍ قابلٍ للتخصيص |
| `tests/review-round4.mjs` (جديد) · `tests/run.mjs` | كل بندٍ يُقاس |

### ج. لم يُمسّ

- **بيانات النسخة والمزامنة** — تغيّر متى يُعرض الشريط، لا ما يُحفظ.
- **أزرار الجوّال في جدول العملاء** (المرحلة ٥٧).
- **الطباعة** — البطاقةُ إلى جانبها لا بدلَها.

## ٦٨. المرحلة ٦١ — التفريغ: لا شيء «قيد المعالجة»

سأل صاحبُ المكتب: لماذا مستنداتٌ «تحت المراجعة منذ أيام ولم تتمّ»؟ والجواب أنّ **النظام لا
يراجع شيئًا**: التفريغُ فوريّ — النصُّ الملصوق يُقرأ في لحظته، والملفُّ يُرسل إلى مزوّده ويعود
بنصّه أو بسبب فشله. وما كان يحمل شارةَ «بانتظار المراجعة» أيامًا هو أحدُ اثنين: سجلٌّ مفرَّغٌ
**ينتظر اعتمادَه هو**، أو ملفٌّ **فشل تفريغُه** (لا مزوّدَ لقراءة الصور مهيَّأ) وحمل الشارةَ
نفسَها فوق سطر الخطأ — فقُرئ انتظارًا. الشارةُ كانت العطب.

### أ. ما تغيّر

- **الشارةُ بلفظها**: «تنتظر اعتمادك» للمفرَّغ، و«لم يُفرَّغ — يحتاج تدخّلك» بالأحمر للفاشل
  (ولا يحمل شارةَ انتظارٍ أبدًا)، و«معتمَدة». والفلاتر كذلك، و«تنتظر اعتمادك» لا يعدّ الفاشل.
- **«✓ اعتمد» في رأس البطاقة** بارزًا — كان «راجعتُه» خافتًا في آخرها.
- **وصفُ اللوحة** يقولها: التفريغُ فوريّ، وما ينتظر هو اعتمادُك.
- **ملفّاتٌ عدّة دفعةً واحدة**: كان الاختيارُ متعدّدًا في الكود ولا يقوله الزرّ ولا يُرى له
  تقدّم. الزرُّ يقولها، وسطرٌ يعدّ «يُفرَّغ ٢ من ٥ — اسم الملف»، وبعدها ملخّصٌ بالعدد: كم فُرّغ
  وكم لم يُفرَّغ. وتُقبل الملفّاتُ بالسحب والإفلات على اللوحة، وبلصق صورةٍ في مربّع النصّ.
- **الصورةُ الفاشلة تُحفظ** (`imageId`، مضغوطةً، الصورُ دون ٨ ميغابايت) كي تُعاد بضغطة
  «↻ أعد المحاولة» بعد تهيئة «قراءة المستندات» — وقبلها الزرُّ معطَّلٌ ويقول لماذا. و«📋 الصق
  نصّه» يقفز إلى مربّع النصّ. (PDF والصوتُ لا يُحفظان: ثقيلان، ويُختاران من جديد.)

### ب. ما لم يتغيّر — ويحتاج قرارًا

قراءةُ الصور تلقائيًّا **ما زالت تحتاج مزوّدًا** (`OCR_API_BASE` و`OCR_API_KEY` و`OCR_MODEL`).
بلا ذلك المسارُ العامل هو «النص المباشر» في الجوّال ثم اللصق. والخياراتُ مع كلفتها في ردّ
المرحلة لصاحب المكتب.

### ج. الملفات المعدَّلة وسببها

| الملف | لماذا |
| --- | --- |
| `js/pages/extract.js` | الشارات والفلاتر · «اعتمد» من الرأس · `takeFiles` بتقدّم · سحب/إفلات/لصق · `keepForRetry` · `retryRecord` |
| `css/components.css` | منطقة الإفلات وسطر التقدّم |
| `tests/extract-batch.mjs` (جديد) · `tests/run.mjs` | ملفّان دفعةً · الشارات · الحفظ للمحاولة |

### د. لم يُمسّ

- **محلّلا الصكوك والطلبات** وما يُقرأ منهما.
- **سجلّات التفريغ القديمة** — تبقى، وتظهر بشاراتها الجديدة.
