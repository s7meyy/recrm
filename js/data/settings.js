...
/**
 * يوزر قصير يُقرأ ويُنطق، مشتقٌّ من المعرّف نفسه (المرحلة ٣٨).
 * المعرّف الكامل يبقى هو المخزَّن والموسوم به في السجلّات — هذا عرضٌ له لا بديلٌ عنه،
 * فاشتقاقه ثابتٌ من الحروف نفسها لا عشوائيّ، ليدلّ الاثنان على واحد.
 */
...
export const DEFAULT_COMPANY = {
  ...
  licenseNumber: '',
  // انتهاء رخصة فال (المرحلة ٤٠): بلا سريانها لا يُوثَّق عقد ولا يُصدَر ترخيص إعلان،
  // فتاريخُها يُسجَّل ليُنبَّه عليك قبله لا لتكتشفه يوم ترفض المنصّة طلبك.
  ...
  // بنود جاهزة للفواتير وعروض الأسعار (المرحلة ٣٨): سطرٌ لكل بند «الوصف|السعر».
  //
  // كان كل بندٍ يُكتب بيده في كل مستند — والوصف نفسه يتكرّر عشرات المرّات ويختلف كتابةً
  // في كلٍّ منها، فلا يُجمع ولا يُقارن. وهذه تجعله اختيارًا من قائمة، ويبقى قابلًا للتعديل
  // في المستند نفسه (القالب لا يحبس مستندًا).
  invoiceProducts: 'عمولة وساطة عقارية|0\nاستشارة عقارية|0\nإعداد عقد إيجار|0\nإدارة أملاك — رسوم شهرية|0\nتقييم عقاري|0\nتسويق عقار|0',
  // مسار الصفقة (المرحلة ٢٤): قائمة تحقّق تُنسخ إلى كل صفقة جديدة. سطر لكل بند.
  dealChecklist: 'اتفاقية الوساطة موقّعة\nصورة هوية الطرفين\nصورة الصك أو العقد\nتوثيق العقد\nاستلام العمولة',
  // طلب التقييم بعد الصفقة (المرحلة ٢٥): رابط صفحتك في خرائط قوقل أو غيرها.
  // فارغ = لا تظهر لوحة طلب التقييم أصلًا — لا زرّ يرسل عميلك إلى لا شيء.
  reviewUrl: '',
