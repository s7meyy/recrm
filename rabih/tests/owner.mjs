...
/* الكسبُ يُقال في إطاره: «من 4.2 إلى 4.25» على تقييمات قوقل، لا «+1.25 نجمة»
   محسوبةً على ثمانية تعليقات وموضوعةً بجانب متوسطٍ على مئتين وأربعين. */
const gainHtml = briefBlock(p1, jb);
/(وأثرُ هؤلاء على متوسطك|وما تكسبه في عيّنتك)/.test(gainHtml)
  ? ok('يُقال ما يكسبه إن عالج، لا ما يخسره فقط') : bad('بلا مكسب');
!/\+\d+(\.\d+)? نجمة/.test(gainHtml)
  ? ok('ولا «+نجمة» مجرّدةً تُقرأ على متوسطٍ غير متوسطها') : bad('نجمةٌ موهِمة');
/من \d(\.\d+)? إلى \d(\.\d+)?/.test(gainHtml)
  ? ok('بل «من كذا إلى كذا» في إطارٍ مسمًّى') : bad('بلا إطار');
